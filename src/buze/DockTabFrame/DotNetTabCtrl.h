...
protected:
	...
	// for remembering focused colors
	COLORREF m_clrBackgroundFocused;
	COLORREF m_clrTextInactiveTabFocused;
	COLORREF m_clrSelectedTabFocused;

	// for remembering unfocused colors
	COLORREF m_clrBackgroundUnfocused;
	COLORREF m_clrTextInactiveTabUnfocused;
	COLORREF m_clrSelectedTabUnfocused;
...
public:
	...
	LRESULT OnSettingChange(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
	{
		...
		// Calculate Colors
		{
			...
			// until I came up with a formula that seems to pretty well match.
			{
				...
				m_clrBackgroundUnfocused = RGB(nRed, nGreen, nBlue);
				m_clrBackgroundFocused   = RGB(0x18,0x18,0x18);//::GetSysColor(COLOR_BTNHILIGHT);
			}
			...
			// the tab background, only instead of lightening BNTFACE, it darkens GRAYTEXT.
			{
				...
				m_clrTextInactiveTabUnfocused = RGB(nRed, nGreen, nBlue);
				m_clrTextInactiveTabFocused   = ::GetSysColor(COLOR_GRAYTEXT);
			}

			// The background color of the selected tab
			{
				m_clrSelectedTabUnfocused = ::GetSysColor(COLOR_BTNFACE);
				m_clrSelectedTabFocused = ::GetSysColor(COLOR_BTNFACE);
			}
		}
	...
	void SetColors()
	{
		...
		if (!m_EightBPP) {
			if (m_isFocused) {
				m_hbrBackground.CreateSolidBrush(m_clrBackgroundFocused);
				m_clrTextInactiveTab = m_clrTextInactiveTabFocused;
				m_clrSelectedTab = m_clrSelectedTabFocused;
			} else {
				m_hbrBackground.CreateSolidBrush(m_clrBackgroundUnfocused);
				m_clrTextInactiveTab = m_clrTextInactiveTabUnfocused;
				m_clrSelectedTab = m_clrSelectedTabUnfocused;
			}
		} else {
			m_hbrBackground.CreateSysColorBrush(COLOR_WINDOW);
