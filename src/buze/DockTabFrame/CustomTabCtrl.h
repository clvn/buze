...
public:
	...
	BOOL MoveItem(size_t nFromIndex, size_t nToIndex, bool bUpdateSelection = true, bool bNotify = true)
	{
		...
		if(bUpdateSelection)
		{
			...
			{
				pT->SetCurSelForce((int)nToIndex);
			}
		}
	...
	BOOL SwapItemPositions(size_t nFromIndex, size_t nToIndex, bool bUpdateSelection = true, bool bNotify = true)
	{
		...
		if(bUpdateSelection)
		{
			...
			{
				pT->SetCurSelForce((int)nFromIndex);
			}
		}
	...
	BOOL DeleteItem(size_t nItem, bool bUpdateSelection = true, bool bNotify = true)
	{
		...
		{
			pT->SetCurSelForce(nPostDeleteSelection);
		}
	...
	*/
	...
	int SetCurSelForce(int nItem, bool bNotify = true)
	{
		return SetCurSel(nItem, bNotify, true);
	}

	int SetCurSel(int nItem, bool bNotify = true, bool bForce = false)
	{
		T* pT = static_cast<T*>(this);
		ATLASSERT(::IsWindow(m_hWnd));
		...
		// Send notification
		NMCTC2ITEMS nmh = {{ m_hWnd, this->GetDlgCtrlID(), CTCN_SELCHANGING }, iOldSel, nItem, {-1,-1}, bForce ? true : (nItem != m_iCurSel)};
		if(bNotify)
		{
			if( TRUE == ::SendMessage(GetParent(), WM_NOTIFY, nmh.hdr.idFrom, (LPARAM)&nmh) )
			{
