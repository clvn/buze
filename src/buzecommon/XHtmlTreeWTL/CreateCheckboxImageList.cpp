...
/*1111*/_T("tri-state disabled"),	CBS_MIXEDDISABLED,		DFCS_BUTTON3STATE | DFCS_CHECKED | DFCS_INACTIVE | DFCS_FLAT,
	...
	if (pDC.m_hDC && (nSize > 0)) {
		...
		///if (bmpCheckboxes.CreateCompatibleBitmap(pDC, nBmpWidth * nImages, nBmpHeight)) {
		if (bmpCheckboxes.CreateCompatibleBitmap(pDC, nBmpWidth * nImages, nBmpHeight)) {
			if (imagelist.Create(nBmpWidth, nBmpHeight, ILC_COLOR32 | ILC_MASK, nImages, 1)) {
				CDC dcMem;
				if (dcMem.CreateCompatibleDC(pDC)) {
					HTHEME hTheme = NULL;
					hTheme = (g_xpStyle.IsThemeActive() && g_xpStyle.IsAppThemed()) ? g_xpStyle.OpenThemeData(NULL, L"BUTTON") : NULL;
					///CBitmap* pOldBmp = dcMem.SelectObject(&bmpCheckboxes);
					CBitmapHandle pOldBmp = dcMem.SelectBitmap(bmpCheckboxes);
					dcMem.FillSolidRect(0, 0, nBmpWidth*nImages, nBmpHeight, crBackground);
					...
					CRect rectImage(nImageLeft, nImageTop, nImageLeft+nImageWidth, nImageTop+nImageHeight);
					...
					for (int i = 0; cbdd[i].pszDesc != NULL; ++i) {
						if (_tcscmp(cbdd[i].pszDesc, _T("unused")) == 0) {
							...
							if (hTheme) {
								g_xpStyle.DrawThemeBackground(hTheme, dcMem, BP_CHECKBOX, cbdd[i].nStateId, &rectImage, NULL);
								g_xpStyle.DrawThemeEdge(hTheme, dcMem, BP_CHECKBOX, cbdd[i].nStateId, &rectImage, 0, 0, NULL);
							}
							else {
								dcMem.DrawFrameControl(&rectImage, DFC_BUTTON, cbdd[i].nState);
							}
						}
