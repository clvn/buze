...
static const int XHTMLDRAW_MAX_TEXT = 1000;
static const DWORD COLOR_NONE = ((DWORD)-1);
...
class CXHtmlDraw
{
  public:
	...
	struct XHTMLDRAWSTRUCT
	{
		...
		COLORREF crBackground;			// background color for window - used to fill entire drawing rect
		COLORREF crTextBackground;		// text background color - used for background of text
		HDC		hDC;					// saved dc - used to preserve display area, in case there are links and bTransparent = TRUE
		...
		UINT	uFormat;				// Specifies the method of formatting the text.  See DrawText() for values
		TCHAR*	pszAnchor;				// appcommand for <A> tag
		...
		UINT	nID;					// id to allow you to uniquely identify this CXHtmlDraw object
	};
  ...
  public:
	...
	int Draw(HDC hdc, LPCTSTR lpszText, XHTMLDRAWSTRUCT* pXHDS, BOOL bUnderlineUrl);
