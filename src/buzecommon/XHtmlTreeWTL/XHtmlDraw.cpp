...
// Some common character entities - note that these will be displayed correctly ONLY if they are in the currently selected font
...
int CXHtmlDraw::Draw(HDC hDC, LPCTSTR lpszText, XHTMLDRAWSTRUCT* pXHDS, BOOL bUnderlineUrl)
{
	...
	// megz: commented out these checks because it didn't work with double buffered painting
	...
	if (pXHDS->bTransparent && !pXHDS->hDC) {
		...
		BitBlt(pXHDS->hDC, 0, 0, nRectWidth, nRectHeight, hDC, rectDraw.left, rectDraw.top, SRCCOPY);
		BitBlt(hMemDC, 0, 0, nRectWidth, nRectHeight, hDC, rectDraw.left, rectDraw.top, SRCCOPY);
	}
	else if (pXHDS->bTransparent && pXHDS->hDC) {
		// restore the original drawing area from saved HDC
		BitBlt(hMemDC, 0, 0, nRectWidth, nRectHeight, pXHDS->hDC, 0, 0, SRCCOPY);
	}
	...
	TCHAR ent[3] = { _T('\0') };
	ent[0] = _T('\001'); // each entity name is replaced with a two-character code that begins with \001

	BOOL bCharacterEntities = FALSE; // assume no char entities

	// we are replacing character entites with a two-character sequence, so the resulting string will be shorter
	...
	while ((n > 0) && pszText && (pszText < (pTextBuffer + textLen))) {
					...
					if (_tcsnicmp(szAttributes, _T("COLOR"), 5) == 0) {
						...
						if (cp2) {
							...
							if (cp2) {
								...
								memset(&lf.lfFaceName, 0, nFaceSize);
								_tcsncpy(lf.lfFaceName, szAttributes, (nNewFaceSize > nMaxFaceSize) ? nMaxFaceSize : nNewFaceSize);
						...
						while ((_tcslen(szAttributes) > 0) && (szAttributes[0] != _T(' '))) {
							--m;
							_tcscpy(szAttributes, &szAttributes[1]);
						}
					}
		...
		if ((crBkgndNew != crBackground) && (pXHDS->crTextBackground == COLOR_NONE)) {
	...
	BitBlt(hDC, rectDraw.left, rectDraw.top, nRectWidth, nRectHeight, hMemDC, 0, 0, SRCCOPY);			
...
int CXHtmlDraw::GetPlainText(const TCHAR* html, TCHAR* plain, DWORD dwPlainSize)
{
	size_t nSize = 0;
