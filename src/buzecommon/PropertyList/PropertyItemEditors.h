...
public:
   ...
   BEGIN_MSG_MAP(CPropertyEditWindow)
      ...
      MESSAGE_HANDLER(WM_USER_PROPEDIT_ISEDITDIRTY, OnIsEditDirty)
   END_MSG_MAP()
   ...
   LRESULT OnKeyDown(UINT /*uMsg*/, WPARAM wParam, LPARAM /*lParam*/, BOOL& bHandled)
   {
      ...
      case VK_ESCAPE:
         m_fCancel = true;
         ::PostMessage(GetParent(), WM_USER_PROP_CANCELPROPERTY, (WPARAM)TRUE, (LPARAM)m_hWnd);
         return 0;
      case VK_RETURN:
         m_fCancel = true;
         ::PostMessage(GetParent(), WM_USER_PROP_UPDATEPROPERTY, (WPARAM)TRUE, (LPARAM)m_hWnd);
   ...
   LRESULT OnKillFocus(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& /*bHandled*/)
   {
      ...
      ::PostMessage(GetParent(), m_fCancel ? WM_USER_PROP_CANCELPROPERTY : WM_USER_PROP_UPDATEPROPERTY, (WPARAM)FALSE, (LPARAM) m_hWnd);
      return lRes;
   }
   ...
   LRESULT OnIsEditDirty(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
   {
      return (LRESULT)(!m_fCancel);
   }
};
...
public:
   ...
   LRESULT OnKillFocus(UINT /*uMsg*/, WPARAM wParam, LPARAM /*lParam*/, BOOL& bHandled)
   {
      if( (HWND) wParam != m_wndButton ) ::PostMessage(GetParent(), WM_USER_PROP_UPDATEPROPERTY, 0, (LPARAM) m_hWnd);
      bHandled = FALSE;
      return 0;
   }
   LRESULT OnKeyDown(UINT /*uMsg*/, WPARAM wParam, LPARAM /*lParam*/, BOOL& bHandled)
   {
      ...
      case VK_ESCAPE:
         ...
         ::PostMessage(GetParent(), wParam == VK_RETURN ? WM_USER_PROP_UPDATEPROPERTY : WM_USER_PROP_CANCELPROPERTY, (WPARAM)TRUE, (LPARAM) m_hWnd);
         ::SetFocus(GetParent());
         break;
      case VK_TAB:
      case VK_UP:
   ...
   LRESULT OnButtonKeyDown(UINT /*uMsg*/, WPARAM wParam, LPARAM /*lParam*/, BOOL& bHandled)
   {
      ...
      case VK_ESCAPE:
         ...
         return 0;
	  case VK_DELETE:
         ::PostMessage(GetParent(), WM_USER_PROP_DELETEPROPERTY, (WPARAM)TRUE, (LPARAM) m_hWnd);
		 return 0;
      }
      bHandled = FALSE;
      return 0;
...
public:
   ...
   // added this so we don't get the I cursor when hovering over the dropdown.
   ...
   {
	   BOOL bDummy;
	   OnButtonClicked(0,0,0,bDummy);
	   return 0;
   }
   LRESULT OnButtonClicked(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
   {
      if (m_bVisible/*m_wndList.IsWindowVisible()*/) {
         m_bVisible = false;
         //m_wndList.ShowWindow(SW_HIDE);
         ::PostMessage(GetParent(), WM_USER_PROP_CANCELPROPERTY, (WPARAM)TRUE, (LPARAM) m_hWnd);///added
         ::SetFocus(GetParent());
      }
      else {
         if( m_cyList == 0 ) {
	     ...
	     // Make sure selected item is still visible after the resize
   ...
   // needed this cause IsWindowVisible() is unreliable -- it will return false if
   // any parent window in the hierarchy is not visible.
   bool m_bVisible;
