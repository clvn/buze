...
class ATL_NO_VTABLE CPropertyListImpl : 
   ...
   public COwnerDraw< T >,
   public CDoubleBufferImpl<CPropertyListImpl<T,TBase,TWinTraits> >
{
public:
   ...
   BOOL IsInplaceEditDirty() const
   {
      if (m_iInplaceIndex != -1 && m_hwndInplace) {
         return (BOOL)::SendMessage(m_hwndInplace, WM_USER_PROPEDIT_ISEDITDIRTY, 0, 0);
      } else {
         return FALSE;
      }
   }
   ...
   {
      ATLASSERT(prop);
      ///if (m_hwndInplace && m_iInplaceIndex == idx) return TRUE;///
      // Destroy old editor
      _DestroyInplaceWindow();
   ...
   void _DestroyInplaceWindow()
   {
      if( m_hwndInplace && ::IsWindow(m_hwndInplace) ) {
         ...
         default:
            ...
            ::SendMessage(m_hwndInplace, WM_CLOSE, 0,0);
         }
      }
      m_hwndInplace = NULL;
      m_iInplaceIndex = -1;
   ...
   BEGIN_MSG_MAP(CPropertyListImpl)
      ...
      MESSAGE_HANDLER(WM_USER_PROP_DELETEPROPERTY, OnDeleteProperty)
      ...
      CHAIN_MSG_MAP(CDoubleBufferImpl<CPropertyListImpl>)
   ...
   {
      m_bDirtyRefocus = true;
      RECT rc = {0,0,1,1};
      InvalidateRect(&rc, FALSE); // ensure OnPaint happens, but nothing invalidated.
      return 0;
   }
   ...
   LRESULT OnLButtonDown(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& /*bHandled*/)
   {
      ...
      BOOL bOutside;
      int clicked_idx = ItemFromPoint(CPoint(GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam)), bOutside);
      if (bOutside) return 0;

      int idx = GetCurSel();
      if (idx != clicked_idx) {
         SetCurSel(clicked_idx);
         idx = GetCurSel();
      }
      ...
      if ((m_dwExtStyle & PLS_EX_NOCOLUMNRESIZE) == 0 && GET_X_LPARAM(lParam) == m_iMiddle + iIndent) {
      ...
      if (idx != -1) {
         ...
         if (::SendMessage(GetParent(), WM_NOTIFY, nmh.hdr.idFrom, (LPARAM) &nmh) == 0) { // Ask owner first
            // Translate into action
            if ((m_dwExtStyle & PLS_EX_CATEGORIZED) && GET_X_LPARAM(lParam) < CATEGORY_INDENT) {
               prop->Activate(PACT_EXPAND, 0);
            }
            else if (GET_X_LPARAM(lParam) < m_iMiddle) {
               if (prop->IsEnabled()) {
                  prop->Activate(PACT_ACTIVATE, lParam);
                  BOOL bDummy;
                  OnSelChange(0, 0, NULL, bDummy);
               }
            }
   ...
   LRESULT OnDblClick(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& /*bHandled*/)
   {
      ...
      if( idx != -1 ) {
         ...
         if( ::SendMessage(GetParent(), WM_NOTIFY, nmh.hdr.idFrom, (LPARAM) &nmh) == 0 ) {
            if (GET_X_LPARAM(lParam) < m_iMiddle) {
               if (prop->IsEnabled()) {
                  prop->Activate(PACT_ACTIVATE, lParam);
               }
            }
   ...
   LRESULT OnMouseMove(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL &bHandled)
   {
      ...
      if( m_iPrevious > 0 ) {
         ...
         if( m_iPrevXGhostBar != x ) {
            if( m_iPrevXGhostBar ) _DrawGhostBar(m_iPrevXGhostBar); ///erase
            m_iPrevXGhostBar = x;
            _DrawGhostBar(m_iPrevXGhostBar); ///draw
         }
         return 0;
      }
	  // Anders: Do not call superclass' mousemove since it does stuff i dont like
      //bHandled = FALSE;
      return 0;
   }
   ...
   LRESULT OnNavigate(UINT /*uMsg*/, WPARAM wParam, LPARAM /*lParam*/, BOOL& /*bHandled*/)
   {
      ...
      case VK_DOWN:
         {
			int new_idx = GetCurSel() + (wParam == VK_UP ? -1 : 1);
			if (new_idx < 0) return 0;
			if (new_idx >= GetCount()) return 0;
            ///_DestroyInplaceWindow();
            SetCurSel(new_idx);
            BOOL bDummy;
            OnSelChange(0, 0, NULL, bDummy);
   ...
   LRESULT OnDeleteProperty(UINT /*uMsg*/, WPARAM wParam, LPARAM lParam, BOOL& /*bHandled*/) {
      HWND hWnd = reinterpret_cast<HWND>(lParam);
      ATLASSERT(hWnd);
      if( !::IsWindow(hWnd) || m_iInplaceIndex == -1 ) return 0;
      ATLASSERT(hWnd==m_hwndInplace);
      ...
      ATLASSERT(prop);
      if( prop == NULL ) return 0;
      NMPROPERTYITEM nmh = { m_hWnd, GetDlgCtrlID(), PIN_ITEMDELETE, prop };
      ::SendMessage(GetParent(), WM_NOTIFY, nmh.hdr.idFrom, (LPARAM) &nmh);
      return 0;
   }
   ...
   LRESULT OnSelChange(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
   {
      ...
      if( idx != -1 ) {
         ...
         if( prop->GetKind() != PROPKIND_CATEGORY && prop->IsEnabled() ) {
            if (!(m_hwndInplace && m_iInplaceIndex == idx))
               _SpawnInplaceWindow(prop, idx);
            prop->Activate(PACT_ACTIVATE, 0);
            if( m_dwExtStyle & PLS_EX_SINGLECLICKEDIT ) {
               if( prop->GetKind() == PROPKIND_EDIT ) 
